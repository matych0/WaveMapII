...
def collate_padding(batch: List[Dict]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    """
    traces_bags: List[torch.Tensor] = [item["traces"] for item in batch]
    durations: torch.Tensor = torch.cat([item["duration"] for item in batch])
    events: torch.Tensor = torch.cat([item["event"] for item in batch])
...
def collate_validation_inference(batch: List[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    filepaths: List[str] = [item[3] for item in batch]  # Extract file paths from the batch
    trace_indices: List[int] = [item[4] for item in batch]  # Extract trace indices from the batch
    ptp: List[torch.Tensor] = [item[5] for item in batch]  # Extract peak-to-peak values from the batch
    ...
    return durations, events, traces_padded_bags, traces_masks, filepaths, trace_indices, ptp
